...
describe('static deployment regressions', () => {
  ...
  it('versions the app shell for this repair so installed cards receive the update', async () => {
    await expect(readFile(projectFile('public/sw.js'), 'utf8')).resolves.toContain("const VERSION = 'mhc-v8'");
    await expect(readFile(projectFile('public/manifest.webmanifest'), 'utf8')).resolves.toContain('"start_url": "/?v=7"');
  });
});
